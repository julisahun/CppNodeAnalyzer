...
describe("simple", () => {
  it('should detect includes', () => {
    const code: string = fs.readFileSync("test/sources/simple/includes.cpp").toString();
  ...
  it('should detect unused variable', () => {
    const code: string = fs.readFileSync("test/sources/simple/unusedVariable.cpp").toString();
    const analyzer: Analyzer = new Analyzer();
    const result: analyzerResult = analyzer.analyze(code);
  ...
  it('should detect redeclaration', () => {
    const code: string = fs.readFileSync("test/sources/simple/redeclaration.cpp").toString();
    const analyzer: Analyzer = new Analyzer();
    const result: analyzerResult = analyzer.analyze(code);
